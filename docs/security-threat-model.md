...
- Account enumeration: identical sign-in failure for unknown username and wrong password, a comparable hashing cost when the user does not exist, and a constant forgot-password response.
...
Authentication gaps accepted for now: no multi-factor authentication, no password-strength check beyond a 12-character minimum, no breached-password check, no account lockout (rate limiting only), no self-service account deletion, and an unverified email address still permits sign-in — which means such an account cannot recover from a lost password.
